...
def buscar_turnos(
    ...
    if dni is not None:
        # 🔎 Corregido: join con Persona para filtrar por DNI
        q = q.join(models.Persona).filter(models.Persona.dni == dni)
...
def obtener_turnos_disponibles(db: Session, fecha: date):
    ...
    ocupados = {t.hora for t in buscar_turnos(
        db, fecha_desde=fecha, fecha_hasta=fecha) if t.estado != ESTADO_TURNO_CANCELADO}
    slots, actual = [], datetime.combine(fecha, inicio)
    while actual.time() < fin:
        hora = actual.time()
        slots.append(schemas.SlotDisponible(
            hora=hora, estado="ocupado" if hora in ocupados else "disponible"))
        actual += intervalo
    return slots
#nuevo

def contar_cancelaciones_por_persona(db: Session, persona_id: int) -> int:
